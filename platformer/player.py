...
class Player(Sprite):
    def __init__(self, x, y, grass_tiles, water_tiles):
        ...
        for i in range(1,9):
            self.move_right_sprites.append(pygame.transform.scale(pygame.image.load(f"assets/boy/Run ({i}).png"), (96,96)))

        for sprite in self.move_right_sprites:
            self.move_left_sprites.append(pygame.transform.flip(sprite, True, False))

        for i in range(1,10):
            self.idle_right_sprites.append(pygame.transform.scale(pygame.image.load(f"assets/boy/Idle ({i}).png"), (96,96)))

        self.idle_left_sprites = [pygame.transform.flip(sprite, True, False) for sprite in self.idle_right_sprites]
